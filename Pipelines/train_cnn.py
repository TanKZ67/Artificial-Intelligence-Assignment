...
from hand_crop_utils import crop_and_mask_hand
...
LABEL_SMOOTHING = 0.1 
RANDOM_SEED = 42
SAMPLES_PER_CLASS = int(os.environ.get("ASL_SAMPLES_PER_CLASS", "1500"))

MODEL_OUT_PATH = "asl_cnn_model.h5"
TFLITE_OUT_PATH = "asl_cnn_model.tflite" 
LABELS_OUT_PATH = "asl_cnn_labels.txt"
...
# --------------------------------------------------------------------------
# 2. Locating the raw dataset and listing samples
# --------------------------------------------------------------------------
def resolve_raw_dataset_dirs():
    configured = os.environ.get("ASL_RAW_DATASET_DIRS", "").strip()
    if configured:
        return [d.strip() for d in configured.split(",") if d.strip()]

    single = os.environ.get("ASL_RAW_DATASET_DIR", "dataset/asl_alphabet_train/asl_alphabet_train")
    if not os.path.isdir(single):
        single = "dataset/asl_alphabet_train"
    if not os.path.isdir(single):
        single = "dataset"
    return [single]
...
def list_samples(dataset_dirs, samples_per_class=SAMPLES_PER_CLASS):
    ...
    if not valid_dirs:
        raise FileNotFoundError(
            ...
            "extract it into the empty 'dataset' folder first."
        )
...
# --------------------------------------------------------------------------
# 3. Building the in-memory hand-crop cache (replaces preprocess_dataset.py)
# --------------------------------------------------------------------------
    ...
    with mp_hands.Hands(static_image_mode=True, max_num_hands=1, min_detection_confidence=0.5) as hands_detector:
        for i, (path, label) in enumerate(zip(filepaths, labels)):
            ...
            if (i + 1) % 500 == 0 or (i + 1) == len(filepaths):
                print(f"  [{tag}] processed {i + 1}/{len(filepaths)} images "
                      f"(hand detected in {detected_count})")

    return images, label_indices


# --------------------------------------------------------------------------
# 4. Augmentation - applied on top of the cached crops, training subset only
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# 5. A Keras Sequence that serves batches straight from the in-memory cache
# --------------------------------------------------------------------------
...
# --------------------------------------------------------------------------
# 6. Model definition - MobileNetV2 backbone + a small custom head.
# --------------------------------------------------------------------------
def build_model(input_shape, num_classes):
    try:
        base_model = MobileNetV2(input_shape=input_shape, include_top=False, weights="imagenet")
    except Exception as exc:  # e.g. no internet access to fetch ImageNet weights
...
def export_tflite(model, path):
    try:
        ...
        print(
            f"Warning: TFLite conversion failed ({exc}). "
            f"main.py requires '{path}' to run - re-run this script or "
            "investigate the error above before trying the real-time demo."
        )
...
def save_history_json(history_head, history_finetune, path):
    ...
    print(f"Saved training history to '{path}' (read by generate_report.py).")
...
def main():
    ...
    print("Run generate_report.py for a classification report, confusion matrix and training curves.")
